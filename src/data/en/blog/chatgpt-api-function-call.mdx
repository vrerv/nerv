...
```graph TD
    ...
    A[user calls Chat API and provides a list of functions] --> B{{AI model decides whether to call a function}}
    B -->|Decides to use the function| C[Returns a function call request response with function name and call parameters]
    B -->|Do not use the function| G[Generate the final response]
    C --> D[User calls a function to generate a response]
    D --> E[Create a new query by adding the function call response value to an existing query]
    E --> F[call the Chat API again]
    F --> H[AI model takes the new query and utilises the function response to generate the final response]
    G --> I[return the final response]
    H --> I[Finalised response]
```
