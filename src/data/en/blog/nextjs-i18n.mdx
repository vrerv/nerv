...
The `i18n` setting previously added to `next.config.js` is replaced by the `next-i18next` setting to eliminate duplication.
...
Modify the hardcoded Text to be imported into the message key using the [`t`](https://react.i18next.com/latest/usetranslation-hook) function.
