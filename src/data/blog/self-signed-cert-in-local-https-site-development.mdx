...
1. 로컬에서 서버에 접속하는 도메인을 hosts 파일에 등록한다.
2. 루트 인증서를 생성한다.
3. 도메인 인증서를 생성한다.
...
아래 repository 를 통해 아래 본 내용을 스크립트로 쉽게 구동할 수 있다.
...
현재 디렉토리 아래에 다음과 같은 디렉토리를 구조로 만들어 관련 파일을 저장한다. docker 구동시 해당 디렉토리를 위치를 넘겨 구동하게 되므로,
만약 위치를 바꾸면 docker 구동시 옵션을 바꿔줘야 한다. nginx 로컬 설치시 관련 디렉토리에 설정을 넣어줘야 한다.
...
docker nginx 컨테이너 안에서 컨테이너 밖의 서버들에 접속하기 위해서는 --add-host 를 통해 "dockerhost" 를 docker 에서 바라보는 ip 를 지정해줘야 한다.
...
`proxy_pass http://dockerhost:3000;` 부분에 docker 로 설치시 `--add-host dockerhost:127.0.0.1` 형태의 옵션의 호스트명과 일치 시킨다.
만약 nginx 를 로컬에 설치하였다면, `localhost` 로 설정한다.
