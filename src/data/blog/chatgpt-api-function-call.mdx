...
OpenAI가 최근 GPT 모델의 주요 업데이트를 발표했는데, 그 중심에 있는 것이 바로 '함수 호출(function calling)' 기능이다.
함수 호출은 OpenAI ChatAPI 에서 AI model 이 사용자의 응답을 생성할 때, 사용자가 정의한 함수를 통하여 더 낳은 응답을 생성할 수 있으면, 그 함수의 호출을 요청하는 응답을 먼저생성하고,
사용자가 그 함수를 호출하여 받은 응답을 ChatAPI 로 추가로 제공하여 질문하면, 해당 함수의 응답을 포함한 최종 응답을 생성하여 사용자에게 제공하는 기능이다.
...
사용자는 함수 정의 목록을 JSON 형식으로 제공하면, AI는 이를 바탕으로 해당 함수를 호출하고 그 결과를 반환하게 된다.
...
2. 모델은 함수를 호출할지 여부를 결정하며, 함수를 호출할 필요가 있다고 판단되며, 사용자가 함수를 호출할 수 있도록 정의된 함수로 부터 함수 호출을 위한 JSON 데이터를 생성하여 반환한다.
3. 사용자는 응답에서 함수 호출이 있으면, 실제로 해당 함수를 호출하여 응답을 얻는다. 함수 호출 결과를 이전 쿼리에 추가하여 새로운 쿼리를 보낸다.
4. AI 모델은 새로운 쿼리를 받아, 함수 호출 결과를 포함한 최종 응답을 생성한다.

아래는 OpenAI에서 제공하는 예제 날씨를 알려주는 함수를 호출하는 예제이다.
OPENAI_API_KEY 환경 변수에 API 키를 설정하고 예제를 실행한다. 주석은 한글로 번역하였다.
