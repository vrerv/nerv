...
기존에 `next.config.js` 에 추가했던 `i18n` 설정은 `next-i18next` 설정으로 대체되어 중복이 제거된다.
...
하드코딩된 text 를 [`t`](https://react.i18next.com/latest/usetranslation-hook) 함수를 사용하여 메세지키로 가져오도록 수정한다.
